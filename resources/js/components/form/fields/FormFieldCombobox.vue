<!--
 * FormFieldCombobox - Combobox com filtragem client-side
 *
 * Componente nativo sem dependências shadcn/vue.
 * Para busca server-side use FormFieldSearchCombobox.
 -->
<template>
    <Field orientation="vertical" :data-invalid="hasError" class="gap-y-1">
        ...
        <div class="relative" ref="containerRef">
            ...
            >
                <span class="truncate">
                    {{ selectedOption ? getOptionLabel(selectedOption) : (column.placeholder || 'Selecione...') }}
                </span>
                    ...
                    <div class="max-h-60 overflow-y-auto p-1">
                        ...
                        >
                            <Check
                                ...
                                :class="internalValue === getOptionValue(option) ? 'opacity-100' : 'opacity-0'"
                            />
...
const filteredOptions = computed(() => {
    if (!search.value.trim()) return normalizedOptions.value;
...
const selectedOption = computed(() =>
    normalizedOptions.value.find((opt) => getOptionValue(opt) === String(internalValue.value ?? '')),
);
...
function selectOption(value: string) {
    internalValue.value = value === String(internalValue.value ?? '') ? null : value;
    close();
}
