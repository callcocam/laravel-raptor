...
const hasSearch = computed(() => props.column.searchable !== false && (props.column.options?.length ?? 0) > 5)
const showSelectAll = computed(() => props.column.showSelectAll !== false && (props.column.options?.length ?? 0) > 3)
...
const handleSelectAllChange = (checked: boolean | 'indeterminate') => {
  // Quando indeterminate ou false, próximo estado é selecionar tudo
  // Quando true, próximo estado é desmarcar tudo
  ...
  if (shouldSelectAll) {
    ...
    const filteredValues = new Set(filteredOptions.value.map(option => String(option.value)))
