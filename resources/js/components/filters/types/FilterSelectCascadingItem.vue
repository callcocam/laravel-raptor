<!--
 * FilterSelect — filtro de seleção usando SelectWithClear nativo do pacote.
 *
 * Recebe a configuração de filtro no padrão SelectFilter::toArray():
 *   { name, label, placeholder, options: [{ value, label }] }
 *
 * Expõe clear nativo, busca opcional e keyboard navigation.
-->
...
<template>
    <SelectWithClear :model-value="modelValue != null ? String(modelValue) : null" :label="filter.label"
        :placeholder="filter.placeholder ?? filter.label" :options="filter.options ?? []" option-value="value"
        option-label="label" :searchable="filter.searchable ?? (filter.options?.length ?? 0) > 6"
